...
private $path;
    ...
    public function switcher() { //FILE_APPEND Если файл filename уже существует, данные будут дописаны в конец файла вместо того, чтобы его перезаписать.
        ...
        if (count($status) == 2) {
            ...
            if ($status[0] === 'work') {
                file_put_contents($this->path . '/work.csv', $status[1] . ',', FILE_APPEND);
            } elseif ($status[0] === 'break') {
                file_put_contents($this->path . '/rest.csv', $status[1] . ',', FILE_APPEND);
            }
        } else {
            file_put_contents($this->path . '/status.csv', 'work,' . gmdate("H:i:s"), FILE_APPEND);
        }
